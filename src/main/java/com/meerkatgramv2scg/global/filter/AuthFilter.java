 ...
 * - 모든 라우팅 요청에 대해서 JWT 검증 및 사용자 식별 헤더를 하위 서비스로 전파시키는 기능
 */
@Component
...
public class AuthFilter implements GlobalFilter, Ordered {
  ...
  public Mono<Void> filter(@NonNull ServerWebExchange exchange, @NonNull GatewayFilterChain chain) {
    try {
      ...
      // 하위 서비스로 전달할 Http Header 세팅
      // ServerRequest가 아닌 ServerHttpRequest를 사용해야 합니다.
      ServerHttpRequest request = exchange.getRequest().mutate()
          .headers(httpHeaders -> httpHeaders.remove(jwtConfig.headerKey())) // 불필요해진 토큰 외부 노출을 막기 위해 제거
  ...
  private Mono<Void> unauthorized(ServerWebExchange exchange) {
    ...
    byte[] bytes = objectMapper.writeValueAsBytes(CustomResponseCode.SCG_INVALID_TOKEN_ERROR);
